...
if __name__ == "__main__":
    ...
    y_train_full = pd.read_csv("y_train.csv").squeeze() # Ensure y is a Series
    ...
    print("\nGenerating predictions for Test Set 1...")
    for model_name in models_to_run:
         print(f"\n--- Running Final: {model_name.upper()} on Test Set 1 ---")
         if model_name == 'mlp':
             pred1 = predict.get_mlp_predictions(X_train_full, y_train_full, X_test1, selector_final, scaler_final, val_mode=False)
         else:
             pred1 = predict.get_predictions(model_name, X_train_full, y_train_full, X_test1, selector_final, scaler_final, val_mode=False)
         final_preds1[model_name] = pred1

    # Create ensemble predictions
    print("\nEnsembling final predictions...")
    ensemble1 = np.mean(list(final_preds1.values()), axis=0)
    print(f"Shape of final predictions for Test Set 1: {ensemble1.shape}")

    # Save results
    output_path1 = os.path.join(OUTPUT_DIR, 'preds_1.npy')
    print(f"\nSaving predictions to {output_path1}...")
    np.save(output_path1, ensemble1)
    ...
    with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zf:
        if os.path.exists(output_path1):
             zf.write(output_path1, 'preds_1.npy')
